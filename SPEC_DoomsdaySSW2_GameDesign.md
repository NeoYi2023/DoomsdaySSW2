...
4. **返回阶段**: 返回避难所保存资源
5. **功能解锁阶段**: 在避难所中消耗资源解锁章节功能，推进主线剧情
...
- **垃圾产出处理规则**: 
  ...
  - **医疗包产出**：如果产出中包含医疗包，医疗包直接添加到探险队的 `medicalKits` 字段中，不占用角色背包空间
...
#### 8.4.1 医疗包系统概述
医疗包是独立的资源类型，不占用角色背包空间，但每次探险队出发探索时携带的医疗包数量有上限限制。医疗包可以在探索棋盘中收集，也可以从避难所带出。

#### 8.4.2 医疗包获取
- **从避难所带出**: 在避难所界面，玩家可以选择携带一定数量的医疗包（不超过携带上限）
- **探索棋盘收集**: 医疗包可以在探索棋盘中作为垃圾产出收集到，与从避难所带出的医疗包存放在同一个存储位置中
- **医疗包存储**: 医疗包存储在探险队的 `medicalKits` 字段中，不占用角色背包
...
- **产出配置**: 在 `GarbageConfig` 中可以配置医疗包作为垃圾产出
...
- **角色强化**: 通过避难所功能解锁后，消耗特定道具对单个角色的属性或技能效果进行一次性提升（如最大体力上限+2）
- **科技树解锁**: 通过避难所功能解锁后，消耗科技点或专属道具解锁科技树节点
- **天赋升级**: 通过避难所功能解锁后，消耗专属道具升级角色天赋
- **装备制作**: 通过避难所功能解锁后，消耗资源制作装备

> 注意：角色强化、科技树解锁、天赋升级等功能需要先在避难所中解锁对应的功能模块，才能使用。
...
避难所系统是游戏的核心系统之一，与任务系统和章节系统紧密配合，用于推进主线剧情。避难所不再是一个固定的建筑，而是一个随章节变化而变更的移动"堡垒"。

**核心设计理念**：
- **移动堡垒**: 避难所随章节推进而移动，每个章节对应不同的避难所形态和功能
- **任务驱动**: 避难所功能解锁与任务系统挂钩，通过完成任务推进主线
- **资源消耗**: 解锁章节功能需要消耗资源，形成资源收集→功能解锁→推进主线的循环
...
#### 10.3.1 移动堡垒机制
- **章节切换**: 当玩家完成章节结束任务，解锁新章节时，避难所会移动到新章节对应的位置
- **形态变化**: 不同章节的避难所可能有不同的外观和功能列表
...
Shelter {
    ...
    ShelterAppearance appearance;      // 当前避难所外观（根据章节变化）
}
...
以下设计已废弃，不再使用：
- ❌ **建设系统**: 不再在地图上建设固定建筑
- ❌ **避难所等级系统**: 不再使用等级系统，改为章节功能系统
- ❌ **设施管理**: 不再管理固定设施，改为功能解锁系统
- ❌ **ShelterLevelConfig**: 不再使用避难所等级配置表
...
#### 10.9.2 功能配置表 (ShelterFeatureConfig)
新建功能配置表，字段定义：
...
### 2024-12-10 避难所系统重大变更

#### 设计理念变更
1. **避难所从固定建筑变为移动堡垒**
   - 避难所不再是一个固定在地图上的建筑
   - 避难所随章节推进而移动，每个章节对应不同的避难所位置
   - 避难所的外观和功能列表随章节变化
...
3. **避难所功能重新定义**
   - 基础功能：角色管理、资源存储、返回点（所有章节通用）
   - 章节功能：按章节解锁，包括角色强化、装备制作、科技研究、天赋升级等
   - 功能解锁需要消耗资源，形成资源收集→功能解锁→推进主线的循环

#### 具体变更内容
- **10.1 避难所功能**：重新定义为基础功能和章节功能
- **10.2 建设系统**：**已废弃**，改为功能解锁系统
- **10.3 避难所等级与解锁逻辑**：**已废弃**，改为章节功能系统
- **新增 10.3 避难所与章节的关系**：移动堡垒机制
- **新增 10.4 功能解锁流程**：详细的解锁条件和流程
- **新增 10.5 避难所数据结构**：更新数据结构定义
- **新增 10.6 避难所界面设计**：功能列表和解锁界面
- **新增 10.7 避难所与任务系统的集成**：任务驱动解锁
- **新增 10.8 废弃的设计**：明确标注已废弃的设计
- **新增 10.9 配置表设计**：功能配置表设计

- **22.2.2 章节解锁机制**：添加避难所移动步骤
- **22.3 章节数据结构**：添加功能ID列表和避难所位置
- **22.4.1 章节配置表**：添加可解锁功能ID列表和避难所坐标
- **22.5.2 章节解锁检查**：添加避难所移动逻辑
- **22.6.4 与避难所系统集成**：新增章节系统与避难所系统的集成说明

- **9.4 资源使用**：更新为章节功能解锁为主
- **1.2 核心循环**：将"建设阶段"改为"功能解锁阶段"

#### 配置表变更
- **废弃**: `ShelterLevelConfig`（避难所等级配置表）
- **新增**: `ShelterFeatureConfig`（功能配置表）
- **扩展**: `ChapterConfig`（添加可解锁功能ID列表和避难所坐标）
...
Chapter {
    ...
    WorldPosition shelterPosition; // 该章节避难所的位置坐标
...
| 避难所X坐标 | number | 该章节避难所在地图上的X坐标 | 10 |
| 避难所Y坐标 | number | 该章节避难所在地图上的Y坐标 | 10 |
...
- **避难所位置随章节切换**：章节切换时，避难所移动到新章节的指定位置
- 探险队位置重置到新章节的避难所位置

#### 22.6.4 与避难所系统集成

- 章节切换时，避难所移动到新章节的指定位置
- 解锁新章节的可解锁功能列表
- 已解锁的功能在后续章节中继续可用
